...
void ExcelFormat::oXLSFormatManager::InitializePredefinedFormats()
{
	...
	_formats[46] = XLS_FORMAT_TIME_H;													// Time
...
ExcelFormat::oXLSFormatManager::oXLSFormatManager(oExcel& xls)
 :	p_xls(&xls),
...
ExcelFormat::oXLSFormatManager::oXLSFormatManager(oExcel *pxls)
 :	p_xls(pxls),
	_next_fmt_idx(FIRST_USER_FORMAT_IDX)	// above last reserved format index 163
{
	 InitializePredefinedFormats();
			// overwrite formats from workbook
	 size_t maxFormats = pxls->workbook_.formats_.size();
	 for(size_t i=0; i<maxFormats; ++i) 
	 {
			int idx = pxls->workbook_.formats_[i].index_;

			_formats[idx] = wstringFromLargeString(pxls->workbook_.formats_[i].fmtstring_);
...
int oXLSFormatManager::get_font_idx(const oExcelFont& font)
{
	int i = 0;
	for(vector<Workbook::Font>::const_iterator it=p_xls->workbook_.fonts_.begin(); it!=p_xls->workbook_.fonts_.end(); ++it,++i)
		if (font.matches(*it))
			return i;

	int font_idx = (int) p_xls->workbook_.fonts_.size();
	p_xls->workbook_.fonts_.push_back(Workbook::Font());
	Workbook::Font& new_font = p_xls->workbook_.fonts_[font_idx];
...
const Workbook::Font& oXLSFormatManager::get_font(const oCellFormat& fmt) const
{
	...
	if (font_idx < p_xls->workbook_.fonts_.size())
		return p_xls->workbook_.fonts_[font_idx];
	else {
//		assert(0); // font_idx out of range
		return p_xls->workbook_.fonts_[0];
	}
}

wstring oXLSFormatManager::get_format_string(const oCellFormat& fmt) const
{
...
int oXLSFormatManager::get_format_idx(const wstring& fmt_str)
{
	...
	p_xls->workbook_.formats_.push_back(Workbook::Format());
	Workbook::Format& format = p_xls->workbook_.formats_.back();
...
int oXLSFormatManager::get_xf_idx(const oCellFormat& fmt)
{
	int i = 0;
	for(vector<Workbook::XF>::const_iterator it=p_xls->workbook_.XFs_.begin(); it!=p_xls->workbook_.XFs_.end(); ++it,++i)
		if (fmt.matches(*it))
			return i;

	 // create a new XF
	int xf_idx = (int) p_xls->workbook_.XFs_.size();
	p_xls->workbook_.XFs_.push_back(Workbook::XF());
	Workbook::XF& xf = p_xls->workbook_.XFs_[xf_idx];
